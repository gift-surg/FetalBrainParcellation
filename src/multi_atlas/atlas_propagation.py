...
def _convert_to_one_hot_and_smooth_seg_prior(segmentation_nii):
    ...
    prior_nii = nib.Nifti1Image(hard_prior_seg_one_hot, segmentation_nii.affine)
    return prior_nii
...
def _register_atlas_to_img(image_nii, mask_nii,
    ...
    if use_affine or affine_only:
        ...
        affine_reg_cmd = 'reg_aladin -ref %s -rmask %s -flo %s -fmask %s -res %s -aff %s -comm -voff' % \
            (img_path, mask_path, atlas_img_seg_path, atlas_mask_path, affine_res_path, affine_path)
        os.system(affine_reg_cmd)

        # Warp the atlas mask
        affine_res_mask_path = os.path.join(save_folder, 'affine_warped_atlas_mask.nii.gz')
        affine_warp_mask_cmd = 'reg_resample -ref %s -flo %s -trans %s -res %s -inter 0 -voff' % \
            (img_path, atlas_mask_path, affine_path, affine_res_mask_path)
    ...
    reg_cmd = 'reg_f3d -ref %s -rmask %s -flo %s -fmask %s -res %s -cpp %s %s' % \
        (img_path, mask_path, affine_res_path, affine_res_mask_path, res_path, cpp_path, reg_options)
...
def _propagate_labels(atlas_seg_nii, image_nii, aff_path, cpp_path, save_folder):
    ...
    if aff_path is not None:
        ...
        cmd = 'reg_resample -ref %s -flo %s -trans %s -res %s -inter 1 -voff' % \
            (image_path, atlas_seg_path, aff_path, aff_warped_seg)
        os.system(cmd)
    else:
    ...
    if cpp_path is not None:
        ...
        cmd = 'reg_resample -ref %s -flo %s -trans %s -res %s -inter 1 -voff' % \
            (image_path, aff_warped_seg, cpp_path, warped_seg)
        os.system(cmd)
    else:
